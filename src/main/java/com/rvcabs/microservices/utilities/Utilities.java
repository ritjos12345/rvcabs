...
import com.rvcabs.microservices.RVCabsApplication;
...
import com.rvcabs.microservices.exception.FieldErrorDetails;
...
public class Utilities {
	...
	public static void main(String[] args) throws Exception {
		...
		/*String myCodeText = "SIVA";
		String filePath = "e:/SivaQR.png";
		String directory = "e:/";
		int height=300;
		int width=300;
		int size = 250;
...
/*		String path = createQrcode(myCodeText,width,height,charSet,fileType);
		...
		String qrText = readQRCode(path,charSet,hintMap);*//*
...
/*		generateHashKey();
		String secret = "aBQU8oxW8xcLoXkBCEsHMJPmKrB3z6HII1ttK1DlVM1OvbUqRfgxVR6WAC802zDz";
		String message = "symbol=LTCBTC&side=BUY&type=LIMIT&timeInForce=GTC&quantity=1&price=0.00940900&recvWindow=5000&timestamp=1564397160000";
	...
	public static ArrayList<String> listOfCardStatus() {
		ArrayList<String> cardStatusList = new ArrayList<>();
		cardStatusList.add(Constants.CARD_STATUS_ACTIVATE);
		cardStatusList.add(Constants.CARD_STATUS_CLOSE);
		cardStatusList.add(Constants.CARD_STATUS_SUSPEND);
		cardStatusList.add(Constants.CARD_STATUS_UNSUSPEND);
		cardStatusList.add(Constants.CARD_STATUS_LOSTSTOLEN);
		cardStatusList.add(Constants.CARD_STATUS_MARK_PFRAUD);
		cardStatusList.add(Constants.CARD_STATUS_UNMARK_PFRAUD);
		cardStatusList.add(Constants.CARD_STATUS_MARK_FRAUD);
		cardStatusList.add(Constants.CARD_STATUS_ENABLE);
		
		return cardStatusList;
	}
	
	public static ArrayList<String> listOfAdjustmentTypes() {
		ArrayList<String> adjustmentList = new ArrayList<>();
		adjustmentList.add(Constants.ADJUSTMENT_CREDIT);
		adjustmentList.add(Constants.ADJUSTMENT_DEBIT);
		
		return adjustmentList;
	}
	
	public static ArrayList<String> listOfProcessors() {
		ArrayList<String> adjustmentList = new ArrayList<>();
		adjustmentList.add(Constants.PAYMENT_PROCESSOR_FIS);
		adjustmentList.add(Constants.PAYMENT_PROCESSOR_ICHOOSE);
	...
	public static void prepareBadRequestExceptionAndThrow(Errors errors) throws BadRequestException {
		//throw new BadRequestException(e.getAllErrors().stream().map(a->a.getDefaultMessage()).collect(Collectors.joining(",")));
		List<FieldErrorDetails> fieldErrorDetailsList = new ArrayList<>();
		errors.getAllErrors().forEach(error -> {
			FieldError fieleError = error instanceof FieldError ? ((FieldError) error) : null;
			FieldErrorDetails fieldError = new FieldErrorDetails(fieleError.getField(), fieleError.getDefaultMessage());
	...
	/*private static void createQRImage(File qrFile, String qrCodeText, int size, String fileType,String charSet)
			throws WriterException, IOException {
		// Create the ByteMatrix for the QR-Code that encodes the given String
		Hashtable<EncodeHintType, ErrorCorrectionLevel> hintMap = new Hashtable<>();
		hintMap.put(EncodeHintType.ERROR_CORRECTION, ErrorCorrectionLevel.L);
		QRCodeWriter qrCodeWriter = new QRCodeWriter();
		BitMatrix byteMatrix = qrCodeWriter.encode(qrCodeText.getBytes(charSet).toString(), BarcodeFormat.QR_CODE, size, size, hintMap);
...
/*	public static String readQRCode(String filePath, String charset, Map hintMap)
	...
	public static ResponseEntity<?> makeRestCallAndGetResponse(Object object,String url,String servletPath,HttpHeaders httpHeaders,HttpMethod method) {
		logger.info("URL:{},servletpath:{}",url,servletPath);
		...
		try {
			...
			if (HttpMethod.GET.equals(method)){
				hmacValue = Utilities.generateHashKey(servletPath, RVCabsApplication.HMAC_VALUE);
			}
			if (HttpMethod.POST.equals(method)) {
				try {
					ObjectMapper mapper = new ObjectMapper();
					String jsonString = mapper.writeValueAsString(object);
					hmacValue = Utilities.generateHashKey(jsonString,RVCabsApplication.HMAC_VALUE);
			...
			if(null==servletPath){
				...
				uri= URI.create(url.concat(servletPath));
			}
			//URI uri = URI.create(url.concat(servletPath));
			RestTemplate restTemplate = new RestTemplate();
			headers.add(Constants.HMAC_PARAM,hmacValue);
			HttpEntity<Object> entity = new HttpEntity<>(object, headers);
			result = restTemplate.exchange(uri, method, entity, Object.class);
		} catch (RestClientException e) {
			throw new InternalServerException("UnExpected Error Occurred, Please try after some time");
		}
	...
	public static List<StatusDto> getStatusList(String status) {
	    List<StatusDto> statusDtoList = new ArrayList<>();
		/*switch(status){
			case "Y" :
                statusDtoList.add(new StatusDto(Constants.STATUS_ACTIVE,Constants.STATUS_ACTIVE_DESCRIPTION));
				statusDtoList.add(new StatusDto(Constants.STATUS_SUSPEND,Constants.STATUS_SUSPEND_DESCRIPTION));
				break;

			case "S" :
				statusDtoList.add(new StatusDto(Constants.STATUS_SUSPEND,Constants.STATUS_SUSPEND_DESCRIPTION));
				statusDtoList.add(new StatusDto(Constants.STATUS_RESUME,Constants.STATUS_RESUME_DESCRIPTION));
				break;

			case "N" :
				statusDtoList.add(new StatusDto(Constants.STATUS_DELETED,Constants.STATUS_DELETE_DESCRIPTION));
				break;

			case "P" :
				statusDtoList.add(new StatusDto(Constants.STATUS_PENDING,Constants.STATUS_PENDING_DESCRIPTION));
				statusDtoList.add(new StatusDto(Constants.STATUS_ACTIVE,Constants.STATUS_ACTIVE_DESCRIPTION));
				break;
	...
	public static String generateHashKey(String message,String secret){
		try {
/*			String secretKey = "aBQU8oxW8xcLoXkBCEsHMJPmKrB3z6HII1ttK1DlVM1OvbUqRfgxVR6WAC802zDz";
			String message = "symbol=LTCBTC&side=BUY&type=LIMIT&timeInForce=GTC&quantity=1&price=0.00940900&recvWindow=5000&timestamp=1564407427000";*/
